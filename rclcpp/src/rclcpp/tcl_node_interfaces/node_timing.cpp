...
:   node_base_(node_base),
    ...
    options_(options)
{
    // std::cout << "NodeTiming name is " << node_base_->get_name() << std::endl;
    ...
    use_tcl_timer_ = node_parameters->declare_parameter(
        "tcl_timing_param.enable_timer",
    ...
    src_tasks_ = node_parameters_->declare_parameter(
        "tcl_sched_param.src_tasks",
        rclcpp::ParameterValue(std::vector<std::string>())
    ).get<std::vector<std::string>>();

    networking_topic_ = node_parameters_->declare_parameter(
        "tcl_sched_param.networking_topic",
        rclcpp::ParameterValue(std::string(""))
    ).get<std::string>();

    // use_tcl_ = message_communication_type_ == -1 ? false : true;
    use_net_ = networking_topic_ == std::string("") ? false : true;
...
bool
NodeTiming::use_tcl_timer() const
{
    return this->use_tcl_timer_;
}

bool
NodeTiming::use_net() const
{
    return this->use_net_;
}
...
std::vector<std::string>
NodeTiming::get_src_tasks() const
{
    return this->src_tasks_;
}

std::string
NodeTiming::get_networking_topic() const
{
    return this->networking_topic_;
}
void
NodeTiming::print_node_sched_info()
{
    ...
    std::cout << "networking_topic : " << this->get_networking_topic() << std::endl;
    std::cout << "source task : ";
    if(src_tasks_.size() > 0){
        std::for_each(src_tasks_.begin(), src_tasks_.end(), [&](auto& src_task){
            std::cout << src_task << " ";
        });
    }
    std::cout << std::endl;
...
void
NodeTiming::create_timing_headers()
{
    if(this->propagator_)
        this->propagator_->create_timing_headers();
}
